...
# python eval.py --config_file mega_nerf/configs/${DATASET_NAME}.yaml  --exp_name $EXP_FOLDER --dataset_path $DATASET_PATH --container_path $MERGED_OUTPUT
# for a standard run, comment the following line when debugging
python -m torch.distributed.run --standalone --nnodes=1 --nproc_per_node $NUM_GPUS eval.py --config_file mega_nerf/configs/${DATASET_NAME}.yaml  --exp_name $EXP_FOLDER --dataset_path $DATASET_PATH --container_path $MERGED_OUTPUT